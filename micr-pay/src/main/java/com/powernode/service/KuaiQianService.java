...
import com.powernode.util.HttpUtil;
...
public class KuaiQianService {
    ...
    public Map<String, String> generateFormData(Integer uid, String phone, BigDecimal rechargeMoney) {
        ...
        String bgUrl = "http://localhost:9000/pay/rece/notify";
        //网关版本，固定值：v2.0,该参数必填。
        String version =  "v2.0";
    ...
    public void kqNotify(HttpServletRequest request) {
        ...
        if (flag) {
            // 可以处理业务逻辑
            // 1. 判断商户号是商家自己的码 2. 判断订单在商家是否存在 3. 判断金额是否一直  4.如果成功，更新用户资金  5. 修改冲值表的记录状态  6. 删除redis中的处理过的订单号
    ...
    private void doQuery(String orderId) {
        ...
        try {
            ...
            // 删除redis中的订单
            stringRedisTemplate.boundSetOps(RedisKey.KEY_ORDERID_SET).remove(orderId);
        } catch (Exception e) {
            e.printStackTrace();
            return;
        }
